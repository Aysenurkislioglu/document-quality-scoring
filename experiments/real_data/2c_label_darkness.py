"""
ADIM 2c (ek) — Karanlık ŞİDDETİNİ etiketler.

GEREKÇE: Occlusion'da olduğu gibi (bkz. 2b_label_severity.py), ikili
(var/yok) "karanlık" bayrağı bu veri setinde yalnızca 2/368 fotoğrafta
işaretlenmişti — ayırt edici bilgi çok az. darkness_score'un neredeyse
her fotoğrafta "en kötü modül" çıkmasının GERÇEK bir aydınlatma sorununu
mu yoksa modülün kendi sınırlı veri çeşitliliğini mi yansıttığını
anlamak için dereceli (hiç/az/çok) bir ground-truth gerekiyor.

Tıpkı 2b_label_severity.py gibi tarayıcı tabanlı (localhost, tamamen
yerel). Çıktı: results/real_data/darkness_severity.csv (yalnızca anon_id
+ karanlık derecesi — görüntü/dosya adı yok).

Kullanım:
    python3 experiments/real_data/2c_label_darkness.py
Sonra tarayıcıda: http://127.0.0.1:5052
"""
...
SEVERITY_PATH = PROJECT_ROOT / "results" / "real_data" / "darkness_severity.csv"

SEVERITY_LABELS = {"hic": "Hiç (iyi aydınlatılmış)", "az": "Az karanlık", "cok": "Çok karanlık"}
...
def load_severity():
    ...
    with open(SEVERITY_PATH, newline="", encoding="utf-8") as f:
        return {int(r["anon_id"]): r["karanlik_siddet"] for r in csv.DictReader(f)}
...
def save_severity(data: dict):
    ...
    with open(SEVERITY_PATH, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["anon_id", "karanlik_siddet"])
...
<style>
  ...
  .b-hic { background:#4caf7d; } .b-az { background:#d9a441; color:#1a1400; } .b-cok { background:#e05252; }
...
{% else %}
  ...
  <form method="post" action="{{ url_for('label') }}">
    ...
    <div class="row">
      <button type="submit" name="sev" value="hic" class="btn b-hic">Hiç</button>
  ...
  <p class="hint">Fotoğraf ne kadar karanlık/kötü aydınlatılmış? Hiç = iyi aydınlatılmış,
    Çok = okumayı zorlaştıracak kadar karanlık. Klavye: 1=Hiç 2=Az 3=Çok, ← önceki</p>
  <script>
    document.addEventListener('keydown', (e) => {
      const map = {'1':'hic','2':'az','3':'cok'};
...
def main():
    ...
    print("Tarayıcıda aç -> http://127.0.0.1:5052")
